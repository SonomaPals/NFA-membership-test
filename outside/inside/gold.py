...
import sys
import outside.inside.eFreeNFA as efNFA
import copy
...
def transformTTable(Dict):
    ...
    symbolsPlusEpsilon = symbols # Copy symbols list
    symbolsPlusEpsilon.append("e") # Add epsilon to symbols
    for x in symbolsPlusEpsilon:
        for y in range(len(dict1)): # for every state and input combination
            r = dict1[y].get(x, None) # state.get(symbol, None) --> 
            if r == None:
                dict1[y][x] = [] # If it doesn't exist in Garret's table, add an empty list for Katie's format
...
def regexToNFA(inp):
    ...
    return transformTTable(Dict) # Dict --> Dict
