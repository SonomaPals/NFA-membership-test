    ...
    def remove_epsilons(self):
        ...
        for i in range(self.num_states):
            ...
            for j in range(self.num_symbols):
                tt[i].append([])
        newNFA = NFA(self.num_states, self.num_symbols, tt) # Create a new NFA with empty transition table
        for k in range(self.num_states):
            for i in epsilon_closures[k]: # For every state in the epsilon closure of state 0
                for j in range(self.num_symbols): # For each of the symbols allowed
                    for item in self.transition_table[i][j]: # For each item in the closure of this state on this input symbol
...
def convert_nfa(eNFA):
    eFreeNFA = eNFA.remove_epsilons()
    return eFreeNFA
